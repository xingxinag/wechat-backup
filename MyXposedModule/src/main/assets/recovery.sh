...
DocFile="${bakDBDir}/WxFileIndex.db"
...
[[ $wechat_versioncode -gt 1681 ]] && StMicroMsgDir="${DataMicroMsgDir}" 
...
[[ -e "$DocFile" ]] && rm -rf ${DataEnPath}/WxFileIndex* && cp -rf "$DocFile" "$DataEnPath" && chmod 0777 "${DataEnPath}/WxFileIndex.db"
...
reMediaFiles 'attachment' "${DataEnPath}/attachment" '文档'
reMediaFiles 'emoji' "${DataEnPath}/emoji" '头像'
chmod -R 0777 "${StMicroMsgDir%/*}"
echo "媒体文件恢复完成！！"
# 恢复 媒体文件 结束
...
# 恢复 登录信息 开始
if [[ -d "${bakAuthDir}/.auth_cache" ]] && [[ -e "${bakAuthDir}/host-redirect.xml" ]] && [[ -e "${bakAuthDir}/systemInfo.cfg" ]] && [[ -e "${bakAuthDir}/autoauth.cfg" ]] && [[ -e "${bakAuthDir}/auth_info_key_prefs.xml" ]] && [[ -e "${bakAuthDir}/app_brand_global_sp.xml" ]];then
unset State
cpAuth(){
rm -rf "${2}/${1}"
...
cpAuth 'app_brand_global_sp.xml' "${wxDataPath}/shared_prefs" '0777'
